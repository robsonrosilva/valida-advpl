...
export class Include {
  private includeExpressoes: IncludeModel[];
  ...
  public valida(objetoValidacao: ValidaAdvpl, conteudoFile: String) {
    ...
    let includesFonte: string[] = objetoValidacao.includes.map(
      (x: any) => x.include
    );
    let includesAnalise: string[] = this.includeExpressoes.map(
      (x: any) => x.include
    );
