...
class Include {
    ...
    valida(objetoValidacao, conteudoFile) {
        ...
        if (!objetoValidacao.includes.indexOf((x) => x.include === "TOTVS.CH")) {
            objetoValidacao.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(0, 0, 0, 0), localize('src.Includes.faltaTOTVS', 'Missing TOTVS.CH !'), vscode_1.DiagnosticSeverity.Warning));
        }
        ...
        objetoValidacao.includes.forEach((include) => {
            ...
            if (this.includesObsoletos.indexOf(include.include) !== -1) {
                objetoValidacao.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(include.linha, 0, include.linha, 0), localize('src.Includes.oInclude', 'The Include ')
                    + include.include +
                    localize('src.Includes.SubstTOTVS', ' is obsolete, it has been replaced by TOTVS.CH!'), vscode_1.DiagnosticSeverity.Warning));
            }
            ...
            if (objetoValidacao.includes.findIndex(function (x) {
                ...
                objetoValidacao.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(include.linha, 0, include.linha, 0), localize('src.Includes.oInclude', 'The Include ')
                    + include.include +
                    localize('src.Includes.emDuplicidade', ' is in duplicity!'), vscode_1.DiagnosticSeverity.Warning));
            }
        });
                ...
                element.expressoes.forEach((expressao) => {
                    if (linha.search(expressao) !== -1) {
                        ...
                        if (includesFonte.indexOf(element.include) === -1) {
                            objetoValidacao.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), localize('src.Includes.faltaInclude', 'Import missing include ') + element.include + '!', vscode_1.DiagnosticSeverity.Error));
                        }
                    }
                });
            });
        ...
        objetoValidacao.includes.forEach((include) => {
            ...
            if (includeAnalise) {
                if (!includeAnalise.precisa) {
                    objetoValidacao.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(include.linha, 0, include.linha, 0), 'Include ' + include.include + localize('src.Includes.desnecessario', ' not necessary!'), vscode_1.DiagnosticSeverity.Warning));
                }
                ...
                includeAnalise.includes.forEach((includeContido) => {
                    ...
                    if (includeAnaliseContido) {
                        objetoValidacao.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(includeAnaliseContido.linha, 0, includeAnaliseContido.linha, 0), localize('src.Includes.oInclude', 'The Include ') +
                            includeAnaliseContido.include +
                            localize('src.Includes.desnecessarioContido', ' unnecessary, is contained in the include ')
                            + include.include + '!', vscode_1.DiagnosticSeverity.Warning));
                    }
                });
            }
        });
