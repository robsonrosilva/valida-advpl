...
class Include {
    ...
    valida(objetoValidacao, conteudoFile) {
        ...
        if (!objetoValidacao.includes.indexOf((x) => x.include === "TOTVS.CH")) {
            objetoValidacao.aErros.push(new erros_1.Erro(0, 0, i18n.__("includes.faltaTOTVS", "Missing TOTVS.CH !"), erros_1.Severity.Warning));
        }
        ...
        objetoValidacao.includes.forEach((include) => {
            ...
            if (this.includesObsoletos.indexOf(include.include) !== -1) {
                objetoValidacao.aErros.push(new erros_1.Erro(include.linha, include.linha, i18n.__("includes.oInclude", "The Include ") +
                    include.include +
                    i18n.__("includes.SubstTOTVS", " is obsolete, it has been replaced by TOTVS.CH!"), erros_1.Severity.Warning));
            }
            ...
            if (objetoValidacao.includes.findIndex(function (x) {
                ...
                objetoValidacao.aErros.push(new erros_1.Erro(include.linha, include.linha, i18n.__("includes.oInclude", "The Include ") +
                    include.include +
                    i18n.__("includes.emDuplicidade", " is in duplicity!"), erros_1.Severity.Warning));
            }
        });
                ...
                element.expressoes.forEach((expressao) => {
                    if (linha.search(expressao) !== -1) {
                        ...
                        if (includesFonte.indexOf(element.include) === -1) {
                            objetoValidacao.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("includes.faltaInclude", "Import missing include ") +
                                element.include +
            ...
            if (includeAnalise) {
                if (!includeAnalise.precisa) {
                    objetoValidacao.aErros.push(new erros_1.Erro(include.linha, include.linha, "Include " +
                        include.include +
                        i18n.__("includes.desnecessario", " not necessary!"), erros_1.Severity.Warning));
                }
                ...
                includeAnalise.includes.forEach((includeContido) => {
                    ...
                    if (includeAnaliseContido) {
                        objetoValidacao.aErros.push(new erros_1.Erro(includeAnaliseContido.linha, includeAnaliseContido.linha, i18n.__("includes.oInclude", "The Include ") +
                            includeAnaliseContido.include +
                            i18n.__("includes.desnecessarioContido", " unnecessary, is contained in the include ") +
                            include.include +
