...
class ValidaAdvpl {
    ...
    validacao(texto, path) {
        ...
        for (var key in linhas) {
            ...
            if (!emComentario) {
                ...
                if (!cBeginSql &&
                    ...
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.queryNoEmbedded', objeto.local), erros_1.Severity.Warning));
                    FromQuery = false;
                    cSelect = false;
                }
                if (linha.search(/(\ |\t|\'|\")+DELETE+(\ |\t)+FROM+(\ |\t)/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.deleteFrom', objeto.local), erros_1.Severity.Warning));
                }
                if (linhaClean.search(/MSGBOX\(/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.msgBox', objeto.local), erros_1.Severity.Information));
                }
                if (linha.search(/GETMV\(+(\ |\t|)+(\"|\')+MV_FOLMES+(\"|\')+(\ |\t|)\)/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.folMes', objeto.local), erros_1.Severity.Information));
                }
                if (linha.search('\\<\\<\\<\\<\\<\\<\\<\\ HEAD') !== -1) {
                    ...
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(nFim), traduz('validaAdvpl.conflictMerge', objeto.local), erros_1.Severity.Error));
                }
                if (linha.search(/(\ |\t|\'|\"|)+SELECT+(\ |\t)/) !== -1 &&
                    linha.search('\\ \\*\\ ') !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.selectAll', objeto.local), erros_1.Severity.Warning));
                }
                if (linha.search('CHR\\(13\\)') !== -1 &&
                    linha.search('CHR\\(10\\)') !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.crlf', objeto.local), erros_1.Severity.Warning));
                }
                ...
                objeto.ownerDb.forEach(banco => {
                    if (cSelect && FromQuery && linha.search(banco) !== -1) {
                        objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.noSchema', objeto.local) +
                            banco +
                            traduz('validaAdvpl.inQuery', objeto.local), erros_1.Severity.Error));
                    }
                });
                if (cSelect &&
                    ...
                    objeto.empresas.forEach(empresa => {
                        ...
                        palavras.forEach(palavra => {
                            if (palavra.search(empresa + '0') !== -1 &&
                                palavra.length === 6) {
                                objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.tableFixed', objeto.local), erros_1.Severity.Error));
                            }
                        });
                    });
                }
                ...
                if ((linha.match(/(\ |\t|)+SELECT+(\ |\t)/) ||
                    ...
                    if (addErro) {
                        objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz('validaAdvpl.bestAnalitc', objeto.local) +
        ...
        if (!comentariosFonte) {
            objeto.aErros.push(new erros_1.Erro(0, 0, traduz('validaAdvpl.padComment', objeto.local), erros_1.Severity.Information));
        }
        ...
        funcoes.forEach(funcao => {
            ...
            if (!achou) {
                objeto.aErros.push(new erros_1.Erro(parseInt(funcao[1]), parseInt(funcao[1]), traduz('validaAdvpl.functionNoCommented', objeto.local), erros_1.Severity.Warning));
            }
        });
        ...
        comentFuncoes.forEach(comentario => {
            ...
            if (!achou) {
                objeto.aErros.push(new erros_1.Erro(parseInt(comentario[1]), parseInt(comentario[1]), traduz('validaAdvpl.CommentNoFunction', objeto.local), erros_1.Severity.Warning));
            }
        });
        ...
        if (objeto.error + objeto.hint + objeto.warning + objeto.information > 0 &&
            this.log) {
