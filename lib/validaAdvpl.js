...
class ValidaAdvpl {
    ...
    validacao(texto, path) {
        ...
        for (var key in linhas) {
            ...
            if (!emComentario) {
                ...
                if (!cBeginSql &&
                    ...
                    objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'Use of Query IMPROPER without Embedded SQL.! \n Use: BeginSQL ... EndSQL.', vscode_1.DiagnosticSeverity.Warning));
                    FromQuery = false;
                    cSelect = false;
                }
                if (linha.search(/(\ |\t|\'|\")+DELETE+(\ |\t)+FROM+(\ |\t)/) !== -1) {
                    objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'Use not allowed use of DELETE FROM!', vscode_1.DiagnosticSeverity.Warning));
                }
                if (linhaClean.search(/MSGBOX\(/) !== -1) {
                    objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'This feature has been deprecated in Protheus 12, use MessageBox()!', vscode_1.DiagnosticSeverity.Information));
                }
                if (linha.search(/GETMV\(+(\ |\t|)+(\"|\')+MV_FOLMES+(\"|\')+(\ |\t|)\)/) !== -1) {
                    objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'This parameter has been deprecated in the Protheus 12!', vscode_1.DiagnosticSeverity.Information));
                }
                if (linha.search("\\<\\<\\<\\<\\<\\<\\<\\ HEAD") !== -1) {
                    ...
                    objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(nFim), 0), 'There are merge conflicts, evaluate before continuing!', vscode_1.DiagnosticSeverity.Error));
                }
                if (linha.search(/(\ |\t|\'|\"|)+SELECT+(\ |\t)/) !== -1 && linha.search("\\ \\*\\ ") !== -1) {
                    objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'Use not allowed use of SELECT with asterisk \n "*".!', vscode_1.DiagnosticSeverity.Warning));
                }
                if (linha.search("CHR\\(13\\)") !== -1 && linha.search("CHR\\(10\\)") !== -1) {
                    objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'It is recommended to use the expression CRLF.', vscode_1.DiagnosticSeverity.Warning));
                }
                ...
                this.ownerDb.forEach(banco => {
                    if (cSelect && FromQuery && linha.search(banco) !== -1) {
                        objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'Use not allowed use of SHEMA '
                            + banco +
                            ' in Query.', vscode_1.DiagnosticSeverity.Error));
                    }
                });
                    ...
                    this.empresas.forEach(empresa => {
                        ...
                        palavras.forEach(palavra => {
                            if (palavra.search(empresa + "0") !== -1 && palavra.length === 6) {
                                objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'NOT ALLOWED Table in Query.', vscode_1.DiagnosticSeverity.Error));
                            }
                        });
                    });
                }
                ...
                if (linhaClean.search(/CONOUT\(/) !== -1) {
                    objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'Use not allowed use of Conout. => Using the Default Log API (FWLogMsg).', vscode_1.DiagnosticSeverity.Warning));
                }
                ...
                if ((linha.match(/(\ |\t|)+SELECT+(\ |\t)/) ||
                    ...
                    if (addErro) {
                        objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(key), 0, parseInt(key), 0), 'To improve the analysis of this query put in different lines the clauses' +
                            ' SELECT, DELETE, UPDATE, JOIN, FROM, ON, WHERE.', vscode_1.DiagnosticSeverity.Information));
                    }
                }
            }
            else {
        ...
        if (!comentariosFonte) {
            objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(0, 0, 0, 0), 'Check patterns of font comments!! => Use autocomplete docFuncao.', vscode_1.DiagnosticSeverity.Information));
        }
        ...
        funcoes.forEach(funcao => {
            ...
            if (!achou) {
                objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(funcao[1]), 0, parseInt(funcao[1]), 0), 'Function, Class, Method or WebService not commented!', vscode_1.DiagnosticSeverity.Warning));
            }
        });
        ...
        comentFuncoes.forEach(comentario => {
            ...
            if (!achou) {
                objeto.aErros.push(new vscode_1.Diagnostic(new vscode_1.Range(parseInt(comentario[1]), 0, parseInt(comentario[1]), 0), 'Function comment without function!', vscode_1.DiagnosticSeverity.Warning));
            }
        });
        ...
        objeto.aErros.forEach((erro) => {
            if (erro.severity === vscode_1.DiagnosticSeverity.Hint) {
                this.hint++;
            }
            if (erro.severity === vscode_1.DiagnosticSeverity.Information) {
                this.information++;
            }
            if (erro.severity === vscode_1.DiagnosticSeverity.Warning) {
                this.warning++;
            }
            if (erro.severity === vscode_1.DiagnosticSeverity.Error) {
                this.error++;
            }
        });
    }
