...
class ValidaAdvpl {
    ...
    validacao(texto, path) {
        this.aErros = [];
        this.includes = [];
        this.fonte = new fonte_1.Fonte(path);
        let objeto = this;
            ...
            if (!emComentario) {
                ...
                if (linhaClean.search(/(STATIC|USER|)+(\ |\t)+FUNCTION+(\ |\t)/) !== -1 &&
                    ...
                    if (linhaClean.search(/(USER)+(\ |\t)+FUNCTION+(\ |\t)/) !== -1) {
                        this.fonte.addFunction(fonte_1.Tipos["User Function"], nomeFuncao, parseInt(key));
                    }
                    else if (linhaClean.split(" ")[0].split("\t")[0] === "FUNCTION") {
                        //verifica se a primeira palavra é FUNCTION
                        this.fonte.addFunction(fonte_1.Tipos["Function"], nomeFuncao, parseInt(key));
                    }
                }
                ...
                if (linhaClean.search("METHOD\\ .*?CLASS") !== -1 ||
                    ...
                    if (linhaClean.split(" ")[0].split("\t")[0] === "CLASS") {
                        this.fonte.addFunction(fonte_1.Tipos["Class"], linhaClean
                            .trim()
                            .split(" ")[1]
                ...
                if (!cBeginSql &&
                    ...
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.queryNoEmbedded", this.local), erros_1.Severity.Warning));
                    FromQuery = false;
                    cSelect = false;
                }
                if (linha.search(/(\ |\t|\'|\")+DELETE+(\ |\t)+FROM+(\ |\t)/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.deleteFrom", this.local), erros_1.Severity.Warning));
                }
                if (linhaClean.search(/MSGBOX\(/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.msgBox", this.local), erros_1.Severity.Information));
                }
                if (linha.search(/GETMV\(+(\ |\t|)+(\"|\')+MV_FOLMES+(\"|\')+(\ |\t|)\)/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.folMes", this.local), erros_1.Severity.Information));
                }
                if (linha.search("\\<\\<\\<\\<\\<\\<\\<\\ HEAD") !== -1) {
                    ...
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(nFim), traduz("validaAdvpl.conflictMerge", this.local), erros_1.Severity.Error));
                }
                if (linha.search(/(\ |\t|\'|\"|)+SELECT+(\ |\t)/) !== -1 &&
                    linha.search("\\ \\*\\ ") !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.selectAll", this.local), erros_1.Severity.Warning));
                }
                if (linha.search("CHR\\(13\\)") !== -1 &&
                    linha.search("CHR\\(10\\)") !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.crlf", this.local), erros_1.Severity.Warning));
                }
                ...
                this.ownerDb.forEach(banco => {
                    if (cSelect && FromQuery && linha.search(banco) !== -1) {
                        objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.noSchema", this.local) +
                            banco +
                            traduz("validaAdvpl.inQuery", this.local), erros_1.Severity.Error));
                    }
                });
                if (cSelect &&
                    ...
                    this.empresas.forEach(empresa => {
                        ...
                        palavras.forEach(palavra => {
                            if (palavra.search(empresa + "0") !== -1 &&
                                palavra.length === 6) {
                                objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.tableFixed", this.local), erros_1.Severity.Error));
                            }
                        });
                    });
                }
                ...
                if (linhaClean.search(/CONOUT\(/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.conout", this.local), erros_1.Severity.Warning));
                }
                ...
                if ((linha.match(/(\ |\t|)+SELECT+(\ |\t)/) ||
                    ...
                    if (addErro) {
                        objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), traduz("validaAdvpl.bestAnalitc", this.local) +
        ...
        if (!comentariosFonte) {
            objeto.aErros.push(new erros_1.Erro(0, 0, traduz("validaAdvpl.padComment", this.local), erros_1.Severity.Information));
        }
        ...
        funcoes.forEach(funcao => {
            ...
            if (!achou) {
                objeto.aErros.push(new erros_1.Erro(parseInt(funcao[1]), parseInt(funcao[1]), traduz("validaAdvpl.functionNoCommented", this.local), erros_1.Severity.Warning));
            }
        });
        ...
        comentFuncoes.forEach(comentario => {
            ...
            if (!achou) {
                objeto.aErros.push(new erros_1.Erro(parseInt(comentario[1]), parseInt(comentario[1]), traduz("validaAdvpl.CommentNoFunction", this.local), erros_1.Severity.Warning));
            }
        });
