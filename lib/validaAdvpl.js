...
class ValidaAdvpl {
    ...
    validacao(texto, path) {
        ...
        for (var key in linhas) {
            ...
            if (!emComentario) {
                ...
                if (!cBeginSql &&
                    ...
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.queryNoEmbedded", "Use of Query IMPROPER without Embedded SQL.! Use: BeginSQL ... EndSQL."), erros_1.Severity.Warning));
                    FromQuery = false;
                    cSelect = false;
                }
                if (linha.search(/(\ |\t|\'|\")+DELETE+(\ |\t)+FROM+(\ |\t)/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.deleteFrom", "Use not allowed use of DELETE FROM!"), erros_1.Severity.Warning));
                }
                if (linhaClean.search(/MSGBOX\(/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.msgBox", "This feature has been deprecated in Protheus 12, use MessageBox()!"), erros_1.Severity.Information));
                }
                if (linha.search(/GETMV\(+(\ |\t|)+(\"|\')+MV_FOLMES+(\"|\')+(\ |\t|)\)/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.folMes", "This parameter has been deprecated in the Protheus 12!"), erros_1.Severity.Information));
                }
                if (linha.search("\\<\\<\\<\\<\\<\\<\\<\\ HEAD") !== -1) {
                    ...
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(nFim), i18n.__("validaAdvpl.conflictMerge", "There are merge conflicts, evaluate before continuing!"), erros_1.Severity.Error));
                }
                if (linha.search(/(\ |\t|\'|\"|)+SELECT+(\ |\t)/) !== -1 &&
                    linha.search("\\ \\*\\ ") !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.selectAll", 'Use not allowed use of SELECT with asterisk "*".!'), erros_1.Severity.Warning));
                }
                if (linha.search("CHR\\(13\\)") !== -1 &&
                    linha.search("CHR\\(10\\)") !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.crlf", "It is recommended to use the expression CRLF."), erros_1.Severity.Warning));
                }
                ...
                this.ownerDb.forEach(banco => {
                    if (cSelect && FromQuery && linha.search(banco) !== -1) {
                        objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.noSchema", "Use not allowed use of SHEMA ") +
                            banco +
                            i18n.__("validaAdvpl.inQuery", " in Query."), erros_1.Severity.Error));
                    }
                });
                if (cSelect &&
                    ...
                    this.empresas.forEach(empresa => {
                        ...
                        palavras.forEach(palavra => {
                            if (palavra.search(empresa + "0") !== -1 &&
                                palavra.length === 6) {
                                objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.tableFixed", "NOT ALLOWED Table in Query."), erros_1.Severity.Error));
                            }
                        });
                    });
                }
                ...
                if (linhaClean.search(/CONOUT\(/) !== -1) {
                    objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.conout", "Use not allowed use of Conout. => Using the Default Log API (FWLogMsg)."), erros_1.Severity.Warning));
                }
                ...
                if ((linha.match(/(\ |\t|)+SELECT+(\ |\t)/) ||
                    ...
                    if (addErro) {
                        objeto.aErros.push(new erros_1.Erro(parseInt(key), parseInt(key), i18n.__("validaAdvpl.bestAnalitc", "To improve the analysis of this query put in different lines the clauses") + " SELECT, DELETE, UPDATE, JOIN, FROM, ON, WHERE.", erros_1.Severity.Information));
                    }
                }
            }
            else {
        ...
        if (!comentariosFonte) {
            objeto.aErros.push(new erros_1.Erro(0, 0, i18n.__("validaAdvpl.padComment", "Check patterns of font comments!! => Use autocomplete docFuncao."), erros_1.Severity.Information));
        }
        ...
        funcoes.forEach(funcao => {
            ...
            if (!achou) {
                objeto.aErros.push(new erros_1.Erro(parseInt(funcao[1]), parseInt(funcao[1]), i18n.__("validaAdvpl.functionNoCommented", "Function, Class, Method or WebService not commented!"), erros_1.Severity.Warning));
            }
        });
        ...
        comentFuncoes.forEach(comentario => {
            ...
            if (!achou) {
                objeto.aErros.push(new erros_1.Erro(parseInt(comentario[1]), parseInt(comentario[1]), i18n.__("validaAdvpl.CommentNoFunction", "Function comment without function!"), erros_1.Severity.Warning));
            }
        });
